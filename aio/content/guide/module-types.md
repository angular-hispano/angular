...
Para ver la aplicación de ejemplo utilizada en temas relacionados con NgModules, consulta el <live-example name="ngmodules"></live-example>.
...
Puedes organizar tus otros NgModules de la manera que deseas.
...
Utiliza un NgModule de dominio para ofrecer una experiencia de usuario dedicada a una función o dominio de aplicación en particular, como editar un cliente o realizar un pedido.
...
Si lo haces, la vida útil de los servicios proporcionados debe ser la misma que la vida útil del NgModule.

<div class="alert is-helpful">

Para obtener más información sobre los ciclos de vida, consulta [Conexión en el ciclo de vida del componente](guide/lifecycle-hooks "Conexión en el ciclo de vida del componente").
...
Utiliza un NgModule enrutado para todos los [NgModules con carga diferida](guide/lazy-loading-ngmodules "Carga diferida de un NgModule").
Utiliza el componente superior del NgModule como destino de una ruta de navegación del enrutador.
...
Si incluyes proveedores, la vida útil de los servicios prestados debe ser la misma que la vida útil del NgModule.
...
Para obtener más información sobre proveedores y NgModules enrutados con carga diferida, consulta [Limitar el alcance del proveedor](guide/providers#limiting-provider-scope-by-lazy-loading-modules "Proporcionar dependencias: limitar el alcance del proveedor").
...
Utiliza un NgModule de enrutamiento para proporcionar la configuración de enrutamiento para un NgModule de dominio, separando así las preocupaciones de enrutamiento de su dominio complementario NgModule.
...
Para obtener una descripción general y detalles sobre el enrutamiento, consulta [Navegación en la aplicación: enrutamiento a vistas](guide/router "Navegación en la aplicación: enrutamiento a vistas").

</div>

Utiliza un NgModule de enrutamiento para realizar las siguientes tareas:

* Definir rutas.
* Agrega la configuración del enrutador a la importación de NgModule.
* Agrega proveedores de servicios de guardia y resolución a los proveedores de NgModule.
...
Importa un NgModule de enrutamiento solo en su NgModule complementario.
...
No uses declaraciones en un NgModule de enrutamiento.
...
Utiliza un NgModule de servicio para proporcionar un servicio de utilidad, como acceso a datos o mensajería.
...
Utiliza sólo la raíz `AppModule` para importar el servicio NgModules.
...
Para aprender a usar módulos compartidos para organizar y optimizar su código, consulta [Compartir NgModules en una aplicación](guide/sharing-ngmodules "Compartir NgModules en una aplicación").

## Sigueintes pasos

También te puede interesar lo siguiente:

* Para obtener más información sobre NgModules, consulta [Organizar su aplicación con NgModules] (guía / ngmodules "Organizar su aplicación con NgModules").
* Para obtener más información sobre el NgModule raíz, consulta [Lanzamiento de una aplicación con un NgModule raíz] (guía / bootstrapping "Lanzamiento de una aplicación con un NgModule raíz").
* Para obtener más información sobre los NgModules angulares de uso frecuente y cómo importarlos a su aplicación, consulta [Módulos de uso frecuente] (guía/módulos de ng frecuentes "Módulos de uso frecuente").
* Para obtener una descripción completa de las propiedades de los metadatos de NgModule, consulta [Uso de los metadatos de NgModule] (guía / ngmodule-api "Uso de los metadatos de NgModule").

Si deseas administrar la carga de NgModule y el uso de dependencias y servicios, consulta lo siguiente:

* Para obtener más información sobre cómo cargar NgModules con entusiasmo cuando se inicia la aplicación, o cómo cargar NgModules de forma perezosa de forma asíncrona por el enrutador, consulta [Lazy-loading feature modules](guide/lazy-loading-ngmodules).
* To comprender cómo proporcionar un servicio u otra dependencia para su aplicación, consulta [Proporcionar dependencias para un NgModule](guide/providers "Proporcionar dependencias para un NgModule").
* Para aprender a crear un servicio singleton para usar en NgModules, consulta [Making a service a singleton](guide/singleton-services "Haciendo un servicio un singleton").